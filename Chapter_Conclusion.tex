...
\chapter{Epilogue}
	...
	\end{figure}
	%
	Figure~\ref{fig:EMC_summary} shows the EMC effect for various nuclei.
	...
	for by the evolution of the operator.  Our results in
	Subsec.~\ref{subsec:operator_evolution} suggest that
	the relationship in Fig.~\ref{fig:EMC_SRC_correlation} might be explained
	by both the quantities being dominantly given by the leading
	two-body (contact) operator.  Studies analogous to the one
	...
	involve overlap of nuclear wave functions.  The short-range parts of the
	wave functions are scale and scheme dependent
	(cf.~Fig.~\ref{fig:SRG_evolution_wf}) and therefore so are the spectroscopic
	factors.
