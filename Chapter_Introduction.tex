...
\chapter{Introduction}
  ...
  references for details.
	...
	R.\ Furnstahl, and R.\ Perry \cite{Bogner:2006pc}.  The basic idea in SRG
	...
	Note that since we just doing unitary transformations, eigenvalues such
	...
	filter.  This discussion also makes it clear that
	nuclear potential itself is not an observable and there is no ``correct''
	nuclear potential.  In fact, there are more than one ``correct'' potential.
	...
	\end{figure}
	%
	Since the SRG evolved potentials only have a low-momentum modes, they are
	...
	to keep the induced 3-body forces \cite{Jurgenson:2010wy}.
	It has been demonstrated that 3-body forces are crucial in getting the
	correct experimental values from theory \cite{Hammer:2012id}.
	The initial
	potential in Fig.~\ref{fig:SRG_Nmax_convergence_He4_Li6}
	includes both two- and three-body forces.  A major development
	...
	For example, $\Phi$ can be a Slater determinant which is fair
	...
	We have seen that nuclear theory has come a long way from pion theories
	of $1940$'s.  The focus these days is on doing precision calculations and
	...
	strengths.  In other cases, alternate methods provide a means to
	cross-check answers.
	Having made great strides in evaluation
	of the bound state properties, the push recently has been on
	calculating resonant and scattering states, and use the improved
	...
	neutrino (i.e, if neutrino is a Majorana or a Dirac fermion).
	...
	These experiments need guidance from theory to look in the right
	parameter space.  The theoretical calculation of $0 \nu \beta \beta$
	...
	\begin{figure}[htbp]
	 ...
	 methods used for calculations.  Figure from Petr Vogel.}
	...
	accompanied by reliable uncertainty estimate.  The source of uncertainty in
	...
	The next two chapters in this thesis will present author's original work as a
	PhD student.
	...
	Theoretical calculations of nuclear cross section involves evaluating
	nuclear structure (which involves description of initial and final state) and
	nuclear reaction (which involves description of probe).  To make
	...
	Chapter~\ref{chap:factorization} by using SRG to look at the scale
	...
	Both these projects were first of its kind, and are fertile grounds for
	...
	by the number of articles citing our publications.  We believe that
	work in Chapter~\ref{chap:factorization} will also receive a wide
	attention soon.  We present the details of our calculations in Appendix, which
	would enable those interested to reproduce our results.
