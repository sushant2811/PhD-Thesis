...
\label{Appendix:evolution_details}
  ...
  \end{subalign}
  %
  Here, $G_0^{-1}(z)^{-1} = z - H_0$ is the free Green's function (which does not
  change under the SRG evolution because $H_0$ does not), and $z$ is an arbitrary
  complex energy parameter that is set to $E^\prime + \ii\epsilon$ to recover the
  ...
  \end{subalign}
  %
  Here, $\ket{\psi_f^{(\lambda)}(k)}$ denotes the (evolved) continuum states with
  momentum $k$, and we have $\ket{\psi_f^{(\lambda)}}
