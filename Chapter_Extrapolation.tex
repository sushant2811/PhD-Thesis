...
\end{align}
  ...
  %
	...
	behavior of the induced UV cutoff.  For fixed $N$, Eq.~\eqref{eq:L_i}
  ...
  In Sec.~\ref{sec:IR_story} we worked in the region where the UV errors were
	...
	derived in Appendix of Ref.~\cite{Konig:2014hma}.
	...
	as shown in Ref.~\cite{Konig:2014hma},	Eq.~\eqref{eq:quant-sep} can be put to
	some use using separable approximations.
	...
	particular representation.  From Eq.~\eqref{eq:UPA-op} one immediately sees that
	%
	\begin{equation}
